...
import time
import json
# Importar la función de análisis de texto desde gemma_agent
try:
    # Intentar importar como módulo relativo (cuando se importa desde otro módulo)
    from .gemma_agent import extraer_datos_herramienta
except ImportError:
...
def ensure_format_for_output(data):
    ...
    # Crear un nuevo diccionario con los campos esperados en formato snake_case
    result = {
        "nombre": data.get("nombre", ""),
        "modelo": data.get("modelo", ""),
        "numero_serie": data.get("numeroSerie", data.get("numero_serie", "")),
        "estado": data.get("estado", ""),
        "fecha_adquisicion": data.get("fechaAdquisicion", data.get("fecha_adquisicion", "")),
        "ultimo_mantenimiento": data.get("ultimoMantenimiento", data.get("ultimo_mantenimiento", "")),
        "proximo_mantenimiento": data.get("proximoMantenimiento", data.get("proximo_mantenimiento", "")),
        "ubicacion": data.get("ubicacion", ""),
        "responsable": data.get("responsable", ""),
        "descripcion": data.get("descripcion", "")
    }
    
    print(f"Formato finalizado, campos normalizados a snake_case: {', '.join(result.keys())}")
    return result
...
def process_file(file_path, lang="spa"):
    ...
    try:
        agent_result_raw = extraer_datos_herramienta(texto)
        ...
        return {
            "nombre": "Herramienta sin identificar",
            "modelo": "",
            "numero_serie": "",
            "estado": "Nuevo",
            "fecha_adquisicion": "",
...
def main():
    parser = argparse.ArgumentParser(description='Extracción de texto de PDFs o imágenes con análisis IA')
    ...
    try:
        ...
        if args.raw:
            ...
            if result:
                ...
                expected_fields = ["nombre", "modelo", "numero_serie", "estado", "fecha_adquisicion", 
                                "ultimo_mantenimiento", "proximo_mantenimiento", "ubicacion", 
...
if __name__ == "__main__":
    print("Herramienta de extracción de texto y análisis IA")
    print("-----------------------------------------------")
    main()
