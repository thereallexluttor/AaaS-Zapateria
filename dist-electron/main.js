...
function write_ws_biff8_cell(ba, cell, R, C, opts) {
  ...
  if (cell.bf) write_biff_rec(ba, 6, write_Formula(cell, R, C, opts, os2));
    ...
    case "n":
      var v = cell.t == "d" ? datenum(parseDate(cell.v)) : cell.v;
      write_biff_rec(ba, 515, write_Number(R, C, v, os2));
      break;
    case "b":
    case "e":
    ...
    case "str":
      if (opts.bookSST) {
        ...
        write_biff_rec(ba, 253, write_LabelSst(R, C, isst, os2));
...
app.whenReady().then(() => {
  ...
  ipcMain.handle("save-temp-file", async (event, { fileName, data }) => {
    try {
      ...
      fsSync.writeFileSync(filePath, Buffer.from(data));
  ...
  ipcMain.handle("export-trabajadores-excel", async (event, { headers, data }) => {
    try {
      ...
      utils.book_append_sheet(workbook, worksheet, "Trabajadores");
  ...
  ipcMain.handle("process-producto-text", async (event, rawText) => {
    try {
      ...
      fsSync.writeFileSync(tempFile, rawText, "utf8");
...
`;
        ...
        fsSync.writeFileSync(tempCodeFile, code, "utf8");
        const env = {
          ...process.env,
          PYTHONPATH: servicesDir,
          PYTHONIOENCODING: "utf-8"
